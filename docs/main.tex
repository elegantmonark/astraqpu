...
For a terminal friendly summary instead of JSON:
...
\subsection{Step 13: Firmware Bounds}

The Arduino control unit now has explicit bounds for line length, instruction count, register count, job id length, register name length, and register value length. It also returns clearer \texttt{NACK} reasons such as \texttt{missing\_job\_id}, \texttt{unknown\_op}, and \texttt{register\_value\_too\_long}. This keeps the reference control unit aligned with the host side trust boundary.

\subsection{Step 14: Malformed Input Tests}

Malformed input tests now cover bad Sutra programs, invalid register names, broken architecture JSON, missing architecture gates, non object serial JSON, and invalid trace files. The point is simple: every format AstraQPU accepts should fail clearly when the input is wrong.

\subsection{Step 15: Timeline Summary}

AstraQPU can now export a compact timeline summary from any scheduled program. The summary reports total duration, instruction count, operation counts, channel occupancy, and qubit occupancy. This is useful for debugging the scheduler and is a natural input for a later dashboard.
